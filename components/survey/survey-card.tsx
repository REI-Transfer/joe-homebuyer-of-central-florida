...
// The original one-step question order, minus the questions that moved to Stage 1.
...
interface SurveyCardProps {
  ...
  // Brand name (config.companyName) shown in the TCPA consent text and used as
  // the pixel content_name. Falls back to neutral wording / getBrandName().
  companyName?: string
}
...
export function SurveyCard({ phoneDisplay = "(800) 000-0000", phoneHref = "8000000000", serviceAreas = [], disqualifiedPropertyTypes = ["mobile-home", "land", "other"], disqualifiedOwnershipLengths = [], allowedStates = [], initialAddress, initialStep, motivationV2 = false, companyName }: SurveyCardProps) {
  ...
  const stage2DisqualifyReason = (field: Stage2Field, value: string): string | null => {
    ...
    // Excellent-condition hard DQ (Joe HBCF rule): move-in-ready homes net more
    // on the open market, so block on a not-a-fit screen; the complete lead is
    // never submitted.
